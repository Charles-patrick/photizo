...
import { amenitiesList, priceRanges, propertyStates, propertyTypes } from "@/lib/data";
...
/**
 * Shared search + filter bar for any "browse properties" grid (client
 * dashboard, realtor dashboard, etc.) — the property catalog it filters
 * against always comes from `@/lib/data`, so every dashboard stays in
 * sync automatically.
 */
export default function PropertyFilterBar({ filters, onChange }: PropertyFilterBarProps) {
  return (
    <div>
      ...
      <div className="mt-5 flex flex-col gap-5 rounded-2xl bg-olive-900 px-5 py-5 sm:flex-row sm:items-center sm:gap-6 sm:px-8">
        <FilterSelect
          icon={<Building2 className="h-4 w-4" />}
          label="Property Type"
          value={filters.type}
          onChange={(value) => onChange({ ...filters, type: value })}
          options={[{ value: "", label: "All Types" }, ...propertyTypes.map((t) => ({ value: t, label: t }))]}
        />
        ...
        <FilterSelect
          ...
          onChange={(value) => onChange({ ...filters, priceRangeIndex: Number(value) })}
          options={priceRanges.map((range, i) => ({ value: String(i), label: range.label }))}
        />
        <div className="hidden h-10 w-px bg-gold-50/15 sm:block" />
        <FilterSelect
          ...
          options={[{ value: "", label: "All Locations" }, ...propertyStates.map((s) => ({ value: s, label: s }))]}
        />
        <div className="hidden h-10 w-px bg-gold-50/15 sm:block" />
        <FilterSelect
          ...
          options={[{ value: "", label: "All Amenities" }, ...amenitiesList.map((a) => ({ value: a, label: a }))]}
        />
  ...
  return (
    <label className="flex min-w-0 flex-1 items-center gap-3">
      ...
      <span className="flex min-w-0 flex-1 flex-col">
        <span className="text-[10px] font-semibold uppercase tracking-wide text-gold-200/80">{label}</span>
        <select
          value={value}
